...
import type {
  ...
  UpdateStyleMessage,
  BatchUpdateItem,
} from '@xagi/vite-plugin-design-mode';
...
export default function IframeDemoPage() {
  ...
  }, [debouncedContent, selectedElement]);
  ...
  }, [debouncedClass, selectedElement]);
      ...
      <div className="flex flex-wrap gap-2">
        {options.map(opt => {
          ...
          const isActive = styleClass
            ? hasStyle(styleClass)
            : !options.some(o => o.value && hasStyle(`${prefix}-${o.value}`));
          return (
            <button
              key={opt.label}
  ...
  const toggleIframeDesignMode = () => {
    ...
    if (iframe && iframe.contentWindow) {
      const message: ToggleDesignModeMessage = {
  ...
  const saveChanges = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        alert(`成功保存 ${result.summary?.successful || pendingChanges.length} 个更改！`);
        setPendingChanges([]);
      } else {
                ...
                <div className="space-y-6">
                  ...
                  <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                    ...
                    <div className="text-xs text-blue-700 space-y-1">
                      <div>
                        <span className="font-medium">标签:</span> {selectedElement.tagName}
                      </div>
                      ...
                      {selectedElement.componentName && (
                        <div>
                          <span className="font-medium">组件:</span>{' '}
                          {selectedElement.componentName}
                        </div>
                      )}
                    </div>
                  ...
                  {selectedElement.isStaticText && (
                    <div>
                      <Label className="mb-2">内容编辑</Label>
                      <Textarea
                  ...
                  <div>
                    ...
                    {renderColorPicker(
                      'bg',
                      ['white', 'gray-100', 'red-100', 'blue-100', 'green-100', 'yellow-100', 'purple-100'],
                      '背景颜色'
                    )}
                    {renderColorPicker(
                      'text',
                      ['gray-900', 'gray-500', 'red-600', 'blue-600', 'green-600', 'purple-600', 'white'],
                      '文字颜色'
                    )}
